...
class _ChatScreenState extends State<ChatScreen> {
  ...
  await  FirebaseFirestore.instance
        .collection('calls').doc(channelName).get().then((value) async {
          ...
          await FirebaseFirestore.instance.collection('calls').add({
            ...
            'status': 'calling',
            'active': true,
          }).then((value) => Navigator.push(context, MaterialPageRoute(builder: (context) => Call(callerName: widget.name!,photo: widget.image!, otherUid: '',isVideoCall: false),)));
  });
  ...
  Future<void> addCollectionAndNavigateToCallScreen({required String reciverId,required bool isVideoCall}) async{
    ...
    Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => Call(callerName: widget.name ?? "",photo: widget.image ?? "", otherUid: reciverId,isVideoCall: isVideoCall),
        ));
